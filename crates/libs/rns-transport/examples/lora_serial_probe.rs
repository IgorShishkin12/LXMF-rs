...
use rns_transport::kiss::{
    encode_command_frame, encode_data_frame, KissCommand, KissFrame, KissStreamDecoder,
};
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("lora_serial_probe: {err}");
        std::process::exit(1);
    }
}
...
async fn run() -> Result<(), String> {
    ...
    let initial_frames: Vec<Vec<u8>> = match lora_config.as_ref() {
    ...
    let shutdown_frames: Vec<Vec<u8>> =
        lora_config.as_ref().map(|lc| lc.shutdown_frames()).unwrap_or_default();

    println!("opening {}...", args.port);
    let mut port = tokio_serial::new(&args.port, args.baud)
        .open_native_async()
        .map_err(|err| format!("open {}: {err}", args.port))?;

    for frame in &initial_frames {
        port.write_all(frame).await.map_err(|err| format!("write failed: {err}"))?;
    }
    println!("probe frames sent");

    let mut decoder = KissStreamDecoder::new(508);
    let mut buf = vec![0u8; 512];
    let mut probe = RNodeProbeStatus::default();
    let mut radio = RNodeRadioStatus::default();

    let startup_end = Instant::now() + Duration::from_millis(args.startup_timeout_ms);
    loop {
        if Instant::now() >= startup_end {
            break;
        }
        match tokio::time::timeout(Duration::from_millis(200), port.read(&mut buf)).await {
            Ok(Ok(n)) if n > 0 => match decoder.push_bytes(&buf[..n]) {
                Ok(frames) => accept_frames(frames, &mut probe, &mut radio, false),
                Err(e) => eprintln!("KISS decode warning: {e:?}"),
            },
            Ok(Ok(_)) | Err(_) => {}
            Ok(Err(e)) => eprintln!("read warning: {e}"),
        }
    }
    ...
    if let Some(hex) = args.send_hex.as_deref() {
        ...
        port.write_all(&frame).await.map_err(|err| format!("send failed: {err}"))?;
        println!("\nsent {} byte(s): {}", bytes.len(), hex::encode(&bytes));
    }

    if args.listen_secs > 0 {
        println!("\nlistening for {} second(s)...", args.listen_secs);
        let listen_end = Instant::now() + Duration::from_secs(args.listen_secs);
        loop {
            if Instant::now() >= listen_end {
                break;
            }
            match tokio::time::timeout(Duration::from_millis(200), port.read(&mut buf)).await {
                Ok(Ok(n)) if n > 0 => match decoder.push_bytes(&buf[..n]) {
                    Ok(frames) => accept_frames(frames, &mut probe, &mut radio, true),
                    Err(e) => eprintln!("KISS decode warning: {e:?}"),
                },
                Ok(Ok(_)) | Err(_) => {}
                Ok(Err(e)) => eprintln!("read warning: {e}"),
    ...
    for frame in frames {
        match frame {
            KissFrame::Data(payload) => {
                if print_packets {
                    println!("received {} byte(s): {}", payload.len(), hex::encode(&payload));
                }
            }
            KissFrame::Command(KissCommand::Unknown(cmd, payload)) => {
    ...
    fn parse<I: IntoIterator<Item = String>>(args: I) -> Result<Self, String> {
        ...
        while let Some(arg) = it.next() {
            match arg.as_str() {
                ...
                "--port" => parsed.port = next_val(&mut it, &arg)?,
                "--baud" => parsed.baud = parse_next(&mut it, &arg)?,
...
fn print_usage() {
    println!(
        "Usage: cargo run -p reticulum-rs-transport --example lora_serial_probe \
-- --port <device> [options]\n\n\
Options:\n  \
--port <device>               Serial device (e.g. /dev/ttyUSB0)\n  \
--baud <rate>                 Baud rate (default 115200)\n  \
--startup-timeout-ms <ms>     Time to collect probe responses (default 2000)\n  \
