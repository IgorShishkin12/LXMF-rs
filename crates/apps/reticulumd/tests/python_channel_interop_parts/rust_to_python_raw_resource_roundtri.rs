...
async fn rust_to_python_backbone_raw_resource_roundtrip() {
    ...
    let server_port = free_tcp_port();
    let temp = tempfile::tempdir().expect("tempdir");
    let py_config_dir = temp.path().join("python-rns-backbone-resource");
    fs::create_dir_all(&py_config_dir).expect("python config dir");
    ...
    let rust_identity = PrivateIdentity::new_from_rand(OsRng);
    let rust_identity = to_transport_private_identity(&rust_identity);
    let mut config = TransportConfig::new("python-backbone-resource-interop", &rust_identity, true);
    config.set_path_request_timeout_secs(2);
    config.set_resource_retry_interval_secs(1);
    let transport = Transport::new(config);
...
#[tokio::test]
#[ignore = "requires local Python Reticulum checkout"]
async fn python_to_rust_backbone_link_data_roundtrip() {
    let _interop_guard = python_interop_guard().await;
    let paths = python_channel_interop_paths();

    let server_port = free_tcp_port();
    let temp = tempfile::tempdir().expect("tempdir");
    let py_config_dir = temp.path().join("python-rns-backbone-link-data-client");
    fs::create_dir_all(&py_config_dir).expect("python config dir");
    write_python_client_config_for_kind(
        &py_config_dir,
        server_port,
    ...
    if !output.status.success() {
        panic!(
            "python Backbone link-data client failed\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
    }
    ...
    assert!(
        stdout.contains("reply:hello-rust"),
        "python Backbone client did not report link-data reply"
    );
}
...
async fn python_to_rust_backbone_request_response_roundtrip() {
    ...
    let server_port = free_tcp_port();
    let temp = tempfile::tempdir().expect("tempdir");
    let py_config_dir = temp.path().join("python-rns-backbone-request-client");
    fs::create_dir_all(&py_config_dir).expect("python config dir");
    write_python_client_config_for_kind(
        &py_config_dir,
        server_port,
    ...
    .await
    .expect("send Backbone request response");
    ...
    if !output.status.success() {
        panic!(
            "python Backbone request client failed\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
    }
    ...
    assert!(
        stdout.contains("reply:hello-rust"),
        "python Backbone client did not report request response"
    );
}
...
#[tokio::test]
#[ignore = "requires local Python Reticulum checkout"]
async fn python_to_rust_backbone_resource_backed_request_response_roundtrip() {
    let _interop_guard = python_interop_guard().await;
    let paths = python_channel_interop_paths();

    let server_port = free_tcp_port();
    let temp = tempfile::tempdir().expect("tempdir");
    let py_config_dir = temp.path().join("python-rns-backbone-large-request-client");
    fs::create_dir_all(&py_config_dir).expect("python config dir");
    write_python_client_config_for_kind(
        &py_config_dir,
        server_port,
    ...
    let mut config = TransportConfig::new(
        "python-backbone-large-request-interop-rust-server",
        &rust_identity,
        true,
    );
    config.set_path_request_timeout_secs(2);
    ...
    if !output.status.success() {
        panic!(
            "python Backbone large request client failed\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
    }
    ...
    assert!(
        stdout.contains("\"reply:large:"),
        "python Backbone client did not report large request response"
    );
}
